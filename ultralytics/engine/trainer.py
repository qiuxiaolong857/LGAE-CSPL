...
class BaseTrainer:
    ...
    def save_model(self):
        ...
        if self.best_fitness == self.fitness:
            torch.save(ckpt, self.best, pickle_module=pickle)  # 始终覆盖，全局当前最优

            best_epoch_path = self.wdir / f"best_epoch{self.epoch + 1}.pt"
            torch.save(ckpt, best_epoch_path, pickle_module=pickle)  # 额外留一份当时快照

            print(f"Best model updated at epoch {self.epoch + 1}, saved to {self.best}")
            print(f"Snapshot saved to {best_epoch_path}")
